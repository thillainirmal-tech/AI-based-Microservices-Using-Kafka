 ...
 * produced by REST controller requests (GET /api/fraud/**).
 *
 * Note: Kafka consumer threads populate their own MDC from TransactionEvent.traceId
 * in TransactionConsumer.consumeTransaction() — this filter only covers HTTP requests.
 *
 * Behaviour:
 *   1. Read X-Trace-Id from the inbound request header.
 *   2. If absent or blank, generate a new UUID v4 as a fallback.
 *   3. Store in MDC under "traceId".
